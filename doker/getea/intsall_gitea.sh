...
rand_hex () {
  if command -v openssl >/dev/null 2>&1; then
    openssl rand -hex "${1:-64}" | tr -d '\n'
  else
    head -c "${1:-64}" /dev/urandom | od -An -tx1 | tr -d ' \n'
  fi
}
...
if ! docker compose version >/dev/null 2>&1; then
  echo "Instale Docker Compose v2 (comando 'docker compose')."
  exit 1
fi
...
# ---------- aguardar responder ----------
need curl
echo "-> Aguardando Gitea subir em ${ROOT_URL} …"
for i in {1..60}; do
  if curl -fsS "${ROOT_URL}" >/dev/null 2>&1; then
    echo "   Gitea está no ar."
    break
  fi
  sleep 2
done
...
echo "Token (também salvo no .env):"
echo "  ${ADMIN_TOKEN:-<não gerado>}"

...
class TaskForm(forms.ModelForm):
    ...
    def __init__(self, *args, project: Optional[Project] = None, **kwargs):
        ...
        if project is not None:
            members = (
                User.objects
                .filter(Q(project_memberships__project=project) | Q(pk=project.owner_id))
                .distinct()
                .order_by("username")
            )
...
class TaskCommitReviewForm(forms.ModelForm):
    ...
    code_quality_text = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"style": "height: 90px", "class": "form-control"}),
    ...
    resolution_text = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"style": "height: 70px", "class": "form-control"}),
...
class KanbanFilterForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["q"].widget.attrs.setdefault("class", "form-control form-control-sm")
